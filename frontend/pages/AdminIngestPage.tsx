...
const AdminIngestPage: React.FC = () => {
  ...
  const handleSimulateWebhook = async () => {
    ...
    try {
        ...
        const parsedJson = JSON.parse(response.text || '{"jobs": []}');
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        <p className="text-slate-500 text-sm mt-1">Manage WhatsApp ingestion and content moderation.</p>
      </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
             ...
             <div className="space-y-3">
                ...
                <div className="pt-3 border-t border-slate-100">
                    <button className="w-full py-2 border border-slate-200 rounded-lg text-xs font-bold text-slate-600 hover:bg-slate-50">
                        Reconnect WhatsApp
                    </button>
                </div>
             </div>
                ...
                <div key={log.id} className="bg-white border border-slate-100 rounded-xl shadow-sm overflow-hidden transition-all hover:shadow-md group">
                  ...
                  {log.status === 'parsed' && (
                    <div className="bg-slate-50 px-6 py-3 border-t border-slate-100 flex justify-end gap-3">
                      <button onClick={() => handleAction(log.id, 'reject')} className="px-4 py-2 text-sm font-medium text-red-600 hover:bg-red-50 rounded-lg transition-colors">
                        Reject
                      </button>
                      <button onClick={() => handleAction(log.id, 'approve')} className="px-4 py-2 text-sm font-medium bg-indigo-600 text-white rounded-lg shadow-sm hover:bg-indigo-700 hover:shadow-md transition-all">
                        Publish as Job
                      </button>
                    </div>
                  )}
